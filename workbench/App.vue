...
const demos = Object.keys(modules)
  .map((p) => ({
    name: p.replace("./demos/", "").replace(".vue", ""),
    load: modules[p],
  }))
...
const selected = ref(
  demos.find((d) => d.name === hashName) ? hashName : (demos[0]?.name ?? ""),
);
const search = ref("");
...
<template>
  <div
    class="flex h-screen overflow-hidden bg-theme-background text-theme-foreground font-sans antialiased"
  >
    <!-- Sidebar -->
    <aside
      class="w-60 shrink-0 flex flex-col border-r border-muted-200 dark:border-muted-800 bg-theme-background"
    >
      <div class="px-4 pt-5 pb-4 space-y-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <div
              class="size-6 rounded-md bg-primary shadow-[0_2px_8px_-2px] shadow-primary/60"
            ></div>
    ...
    <main class="flex-1 overflow-auto">
      <div class="min-h-full p-10">
        ...
        <div
          v-else
          class="flex items-center justify-center h-64 text-theme text-sm"
        >
