...
<template>
  <div class="space-y-10 max-w-lg">
    <section>
      ...
      <div class="space-y-2">
        <p class="text-xs text-theme">Current page: <strong class="text-foreground">{{ page }}</strong></p>
        <UiPagination v-model:page="page" :total="100" :items-per-page="10" :sibling-count="1" show-edges>
          <UiPaginationList v-slot="{ items }">
            <UiPaginationFirst />
            <UiPaginationPrev />
            <template v-for="item in items" :key="item.type === 'page' ? item.value : item.type">
              <UiPaginationListItem v-if="item.type === 'page'" :value="item.value" as-child>
                <UiButton
    ...
    <section>
      <h2 class="text-xs font-semibold uppercase tracking-widest text-theme mb-4">More Siblings</h2>
      <div class="space-y-2">
        <p class="text-xs text-theme">Page: <strong class="text-foreground">{{ page2 }}</strong> of 20</p>
        <UiPagination v-model:page="page2" :total="200" :items-per-page="10" :sibling-count="2" show-edges>
          <UiPaginationList v-slot="{ items }">
            <UiPaginationFirst />
            <UiPaginationPrev />
            <template v-for="item in items" :key="item.type === 'page' ? item.value : item.type">
    ...
    <section>
      <h2 class="text-xs font-semibold uppercase tracking-widest text-theme mb-4">Simple Prev/Next</h2>
      <div class="flex items-center gap-3">
        <UiButton variant="outline" size="sm" :disabled="page <= 1" @click="page = Math.max(1, page - 1)">
          Previous
        </UiButton>
        <span class="text-sm text-theme">Page {{ page }} of 10</span>
        <UiButton variant="outline" size="sm" :disabled="page >= 10" @click="page = Math.min(10, page + 1)">
          Next
        </UiButton>
      </div>
    </section>
