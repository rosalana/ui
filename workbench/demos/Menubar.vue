...
<template>
  <div class="space-y-10 max-w-2xl">
    <section>
      <h2 class="text-xs font-semibold uppercase tracking-widest text-theme mb-4">Application Menubar</h2>
        ...
        <div class="border-b border-muted-200 dark:border-muted-800 px-3">
          <UiMenubar class="border-none shadow-none rounded-none bg-transparent">
            ...
            <UiMenubarMenu>
              ...
              <UiMenubarContent>
                <UiMenubarItem @select="lastAction = 'Documentation'">Documentation</UiMenubarItem>
                <UiMenubarItem @select="lastAction = 'Release Notes'">Release Notes</UiMenubarItem>
                <UiMenubarSeparator />
                <UiMenubarItem @select="lastAction = 'About'">About Rosalana</UiMenubarItem>
              </UiMenubarContent>
            </UiMenubarMenu>
          </UiMenubar>
        </div>

        <div class="p-4 h-32 flex items-center justify-center text-sm text-theme bg-muted-50 dark:bg-muted-950">
          <div class="text-center">
            <p>Zoom: <strong class="text-foreground">{{ zoom }}%</strong> · Bookmarks: <strong class="text-foreground">{{ showBookmarks ? 'shown' : 'hidden' }}</strong></p>
            <p v-if="lastAction" class="mt-1">Last action: <strong class="text-foreground">{{ lastAction }}</strong></p>
          </div>
        </div>
      </div>
    </section>
