...
<template>
  <div class="space-y-10 max-w-lg">
    <section>
      <h2 class="text-xs font-semibold uppercase tracking-widest text-theme mb-4">Inline Command</h2>
      <div class="rounded-xl border border-muted-200 dark:border-muted-800 overflow-hidden shadow-sm">
      ...
      <p v-if="selected" class="text-xs text-theme mt-2">Selected: <strong class="text-foreground">{{ selected }}</strong></p>
    </section>

    <section>
      <h2 class="text-xs font-semibold uppercase tracking-widest text-theme mb-4">Command Dialog (Palette)</h2>
      <UiButton @click="dialogOpen = true">
        <UiIcon name="lucide:command" />
        Open Command Palette
        <UiCommandShortcut class="ml-2 text-primary-foreground/70">⌘K</UiCommandShortcut>
      </UiButton>
