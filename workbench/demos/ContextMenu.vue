...
<template>
  <Section title="File Manager" class="w-96">
    <p class="text-xs text-theme mb-4">
    ...
    <UiContextMenu>
      <UiContextMenuTrigger>
        <div
          class="h-48 rounded-xl border-2 border-dashed border-muted-300 dark:border-muted-700 flex flex-col items-center justify-center gap-2 text-theme select-none cursor-context-menu hover:border-muted-400 dark:hover:border-muted-600 transition-colors"
        >
          <UiIcon name="lucide:folder-open" class="size-8 text-muted-400" />
          <p class="text-sm">Right-click here</p>
          <p class="text-xs" v-if="lastAction">
            Last action:
        ...
        <UiContextMenuSub>
          ...
          <UiContextMenuSubContent>
            <UiContextMenuRadioGroup v-model="sortBy">
              ...
              <UiContextMenuRadioItem value="date"
                >Date Modified</UiContextMenuRadioItem
              >
  ...
  <Section title="Text Editor" class="w-96">
    <p class="text-xs text-theme mb-4">Right-click on the text below.</p>
    <UiContextMenu>
      <UiContextMenuTrigger>
        <p
          class="text-sm p-4 rounded-xl border border-muted-200 dark:border-muted-800 leading-relaxed select-text cursor-context-menu"
        >
          The quick brown fox jumps over the lazy dog. This is a sample text
          that you can right-click to see context menu options for text editing
          operations.
        </p>
      </UiContextMenuTrigger>
